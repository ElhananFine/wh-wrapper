...
class Client extends events_1.EventEmitter {
    constructor(phoneID, token, verifyToken, options = {}) {
        ...
        if ((_d = this.options) === null || _d === void 0 ? void 0 : _d.callbackUrl) {
            if (!this.options.appID || !this.options.appSecret)
                ...
                this._setCallBackUrl(this.options.callbackUrl, this.options.appID, this.options.appSecret);
        }
    }
    async initialize() {
        var _a, _b;
            ...
            try {
                ...
                if (field === "messages") {
                    if ("messages" in value) {
                        this.emit("messages", new message_1.default(this, messageObject));
                    }
                    else if ("statuses" in value) {
                        this.emit("statuses", new MessageStatusHandler_1.default(messageObject));
                    }
                    else {
                    }
                }
    ...
    formatZodError(error) {
        const errorMessages = error.issues.map((issue) => {
            return `(${issue.code}): ${issue.path.join(".")} ${issue.message} expected ${issue.expected} but received ${issue.received}`;
        });
    ...
    async delete_flow() { }
    async sendCatalog() { }
    async sendProduct({ to, catalogId, productSections, title, body, footer, replyToMessageId, }) {
        const action = {
    ...
    async createTemplate(template) {
        ...
        if (typeof template.body === "string") {
            ...
            if (variables && variables.length > 0) {
                variables.map((v, i) => (template.body = template.body
                    .split(v.trim())
                    .join(`{{${i + 1}}}`)));
            }
        ...
        if (template.buttons) {
            ...
            if (template.buttons.filter((b) => (b === null || b === void 0 ? void 0 : b.type) === "PhoneNumberButton").length >
                1)
            ...
            template.buttons.map((button) => {
                switch (button === null || button === void 0 ? void 0 : button.type) {
                    case "UrlButton":
                        ...
                        variables &&
                            variables.map((v, i) => (button.url = button.url.split(v.trim()).join(`{{${i + 1}}}`)));
    ...
    async sendTemplate(to, template, options) {
        ...
        return await this._sendMessage({
            ...
            template: {
                ...
                components: [
                    (template === null || template === void 0 ? void 0 : template.header) && {
                        type: "header",
                        parameters: [
                            {
                    ...
                    (template === null || template === void 0 ? void 0 : template.buttons) &&
                        (template === null || template === void 0 ? void 0 : template.buttons.map((p, i) => {
                            let d = { type: "button" };
    ...
    async makeRequest(config) {
        ...
        try {
            ...
            if ((0, axios_1.isAxiosError)(e)) {
                ...
                throw new errors_1.WhatsAppError(+response.data.error.code, response.data.error.message, response.data.error.fbtrace_id, response.data.error.type, (_a = response.data.error.error_data) === null || _a === void 0 ? void 0 : _a.details);
            }
            else {
                throw new Error(e);
            }
    ...
    async getPhoneNumbers() {
        var _a, _b;
        if (!((_a = this.options) === null || _a === void 0 ? void 0 : _a.businessAccountID))
            throw new Error("You must provide a business account to getPhoneNumbers");
        return await this.makeRequest({
            method: "GET",
            url: `/${(_b = this.options) === null || _b === void 0 ? void 0 : _b.businessAccountID}/phone_numbers/`,
        });
    }
    ...
    async getAllSubscriptions() {
        var _a, _b;
        if (!((_a = this.options) === null || _a === void 0 ? void 0 : _a.businessAccountID))
            throw new Error("You must provide a business account to getAllSubscriptions");
        return (await this.makeRequest({
            method: "GET",
            url: `/${(_b = this.options) === null || _b === void 0 ? void 0 : _b.businessAccountID}/subscribed_apps/`,
        })).data;
    }
        ...
        if (!(options === null || options === void 0 ? void 0 : options.buttons)) {
            return (await this._sendMessage({
                ...
                [type.toLowerCase()]: {
                    ...
                    ...(type === "DOCUMENT" &&
                        (options === null || options === void 0 ? void 0 : options.filename) && { filename: options === null || options === void 0 ? void 0 : options.filename }),
                },
            })).messages[0].id;
        }
        else {
    ...
    async updateBusinessProfile(info) {
        ...
        const data = Object.fromEntries(Object.entries({
            ...
            vertical: (_a = info.industry) === null || _a === void 0 ? void 0 : _a.toLowerCase(),
