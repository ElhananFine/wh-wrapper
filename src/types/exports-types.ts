...
export type Error = { code: number; message: string; details?: string };
export type Options = ClientOptions;
export type GetPhoneData = GetPhoneDataReturn;

export type Media =
    | { mimeType: string; fileSize: string; filePath: string }
    | { mimeType: string; fileSize: string; fileBuffer: Buffer };
